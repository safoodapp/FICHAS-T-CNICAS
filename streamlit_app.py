...
# LEER CADA PESTAÑA ESPECIFICANDO LA URL EN CADA UNA
df_ing = conn.read(spreadsheet=url, worksheet="INGREDIENTES")
df_rec = conn.read(spreadsheet=url, worksheet="RECETAS")
    ...
    if not rec_cliente.empty:
        ...
        if producto_sel:
            ...
            for _, row in df_final.iterrows():
                ...
                # CÁLCULO QUID: Si el ingrediente está en el nombre del producto
            ...
            for col in nutri_cols:
                # Calculamos el valor proporcional de cada nutriente
                totales[col] = round((df_final[col] * df_final['Gramos']).sum() / total_gramos, 2)

            st.table(pd.DataFrame([totales]))
    else:
        st.warning("Este cliente aún no tiene recetas en la pestaña RECETAS.")
else:
    st.error("No se han encontrado clientes en la pestaña CLIENTES.")
